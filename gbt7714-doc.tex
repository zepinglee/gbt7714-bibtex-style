...
@article{chen2026,
  title = {GB/T 7714—2025《信息与文献参考文献著录规则》的修订亮点、规则新点及实施疑点},
  author = {{陈浩元}},
  date = {2026-05-06},
  journaltitle = {编辑学报},
  volume = {38},
  ...
  pages = {150--158},
  issn = {1001-4314},
  url = {http://bjxb.cessp.org.cn/ch/reader/view_abstract.aspx?file_no=20260206&flag=1},
  urldate = {2026-05-06},
}
...
implementation of the Chinese National Standard GB/T 7714 \emph{Information and
...
在有些情况下，序号也可作为行文语句的组成部分\cite[157]{chen2026}。
...
\begin{variable}{citepunct}
  % TODO: 实现 auto 功能。
  \optsyntax[full]{citepunct}{full, half}
...
\begin{variable}{compress}
  \boolsyntax[true]{compress}
  控制是否用短横线连接起讫序号。
  \begin{citeexample}[title={\kvopt{compress}{true}}]
    \textsuperscript{[\highlighta{1-3,5}]}
  \end{citeexample}
  \begin{citeexample}[title={\kvopt{compress}{false}}]
    \textsuperscript{[\highlightb{1,2,3,5}]}
  \end{citeexample}
\end{variable}

\begin{variable}{entrybreak}
  \boolsyntax[true]{entrybreak}
  控制是否允许在一条文献中间分页。
  如果设为 \opt{false} 禁止分页，建议将 \cs{bibsep} 的弹性距离调大一些。
\end{variable}
...
\begin{variable}{bibpunct}
  \label{opt:bibpunct}
  \optsyntax[GB]{bibpunct}{GB, half, bylanguage}
  选择半角/全角符号。
...
\begin{variable}{checkpinyin}
  ...
  带连字符连接的名（如“Li Jiangning”）虽然不符合 GB/T 28039-2011
  《中国人名汉语拼音字母拼写规则》，也按照汉语拼音处理。
...
\begin{variable}{sentencecasejournal}
  \boolsyntax[false (2025, 1987), true (2015, 2005)]{sentencecasejournal}
...
\begin{variable}{journaldots}
  ...
  控制刊名是否添加缩写点。需要在 \field{shortjournal} 字段填写刊名缩写。
  ...
  \begin{bibexample}[title={\kvopt{shortjournal}{true}, \kvopt{journaldots}{true}}]
    \bibitem[Caplan(1993)]{gbt7714.8.1:4}
    Caplan P.
    \newblock Cataloging internet resources\allowbreak[J].
    \newblock \highlightb{Public-Access Comput. Syst. Rev.}，1993，4\allowbreak （2）：61-66.
...
\begin{table}[htb]
  ...
  \begin{threeparttable}[b]
    ...
    \begin{tabular}{clcl}
      ...
      8.13 & 地图             & CM       & \entrytype{map}\tnote{b}                        \\
...
国标的专利文献要求优先著录申请者/所有者（\field{holder}），而非发明者（\field{author}）；
优先著录申请号（\field{applicationnumber}），而不是公开号或公告号（\field{number}），
优先著录公告日期或公开日期（\field{date}），而不是申请日期（\field{origdate}）。
...
@patent{gbt7714.8.10.2:2,
  ...
  title         = {光折变自适应光外差探测方法},
  address       = {中国},
  applicationnumber = {CN01128777.2},
  number        = {CN1338652A},
  year          = {2002},
  date          = {2002-03-06},
  origdate      = {2001-09-05},
