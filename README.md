...
从 v2.0 版本开始（2020-03-04），用户必须在文档中使用 `\bibliographystyle` 命令选择参考文献样式，如 `gbt7714-numeric` 或 `gbt7714-authoryear`。在早期的版本中，选择文献样式的方法是将 `numbers` 或 `super` 等参数传递给 `gbt7714`，而不能使用 `\bibliographystyle`。这跟标准的 LaTeX 接口不一致，所以将被弃用。
...
- 陈浩元：[关于GB/T7714—2015编校失误答同人问](http://bjxb.cessp.org.cn/ch/reader/view_abstract.aspx?file_no=20160138&flag=1)
- 陈浩元：[对GB/T 7714—2015的标识符号问题咨询的答复](http://bjxb.cessp.org.cn/ch/reader/view_abstract.aspx?file_no=20240205&flag=1)
- 陈浩元：[GB/T 7714一2015 的新点、实施要点及其他](http://www.cessp.org.cn/download/90/陈浩元老师：GBT7714一2015的新点、实施要点及其他)（Slides）
